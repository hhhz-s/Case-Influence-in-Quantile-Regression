...
y_ranges <- list(c(0, 0.2), c(0, 0.2), c(0, 4))

y_break_list <- list(c(0,0.1,0.2), c(0,0.1,0.2), c(0,2,4))
...
for (j in 1:length(tau_values)) {
  ...
  for (i in 1:3) {
    ...
    P <- ggplot(dist_df, aes(x = grid_w, y = distance_rescale, color = as.factor(index))) +
      geom_line() +
      scale_y_continuous(limits = common_y_range, breaks = y_breaks, labels = sprintf("%.1f", y_breaks)) +
      theme_minimal() +
      theme(
